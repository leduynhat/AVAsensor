...
bool Store::init(){
    if(!SD.begin(SPI_CS_SD_CARD, SPI))
    {
    ...
    else
    {
        ...
        {
            #ifdef _SERIAL_DEBUG_STORE_
                Serial.print("->SD had no WIFI.txt, now create");
            #endif
...
bool Store::logData(const char* _file_name, DataCore _data_core, char* _mac, TimeNow _date_time)
{
    ...
    if(file)
    {
        if(add_label)
        {
            file.println("Device Id, Date time, Unix Time, Temperature, Humidity, Pressure, PM1, PM2.5, PM10, NO2_W, NO2_A, O3_W, O3_A, CO_W, CO_A, SO2_W, SO2_A, Longitude, Latitude");
        }
        char log_data[300];
        sprintf(log_data, "%s, %4d-%02d-%02d %02d:%02d:%02d, %ld, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.6f, %.6f\n", 
                ...
                _data_core.NO2_WmV, _data_core.NO2_AmV, _data_core.O3_WmV, _data_core.O3_AmV,
                _data_core.CO_WmV, _data_core.CO_AmV, _data_core.SO2_WmV, _data_core.SO2_AmV,
                _data_core.longitude, _data_core.latitude);
        file.print(log_data);
        #ifdef _SERIAL_DEBUG_STORE_
    ...
    if(this->isExist())
    {
        ...
        if(file)
        {
            ...
            while(file.available())
            {
                ...
                #ifdef _SERIAL_DEBUG_STORE_
                    Serial.print("->WiFi info: " + _ssid + ", " + _pwd);
                    // Serial.println(_pwd);
                #endif
                if(!_wifi_mul->addAP(_ssid.c_str(), _pwd.c_str()))
                {
                    #ifdef _SERIAL_DEBUG_STORE_
...
bool Store::logWiFiInfo(String _ssid, String _pass_wd ,const char *_file_name)
{
    if(this->isExist())
    {
        File file = SD.open(_file_name, FILE_APPEND);
