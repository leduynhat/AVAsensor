...
const char* serverUrl = "http://airsense.vn:3030/firmware_ava.bin";
...
{
    #ifdef _SERIAL_DEBUG_WIFI_MANAGER_
...
void WiFiConnectionManager::smartConfig()
{
    ...
    while(millis() < (start_time + 20000))
    {
        if(WiFi.smartConfigDone())
        {
            ...
            // if(device_manager->store.isExist())
            // {
...
void WiFiConnectionManager::getWiFiListFromStore()
{
    ...
    // #endif
    // device_manager->store.readWiFiInfo(&this->wifi_mul);
}
void WiFiConnectionManager::startAP()
{
   //WiFiManager
    ...
    if (!wifiManager.startConfigPortal("AVA sensor V1", "hello123")) {
...
{
    WiFi.disconnect(false,false);
    if(this->wifi_mul.run(connectTimeout) == WL_CONNECTED){
        #ifdef _SERIAL_DEBUG_WIFI_MANAGER_
            Serial.println("->Connect best WiFi SUCCESS");
            Serial.println("->SSID: " + WiFi.SSID());
        #endif
    }
    else{
        #ifdef _SERIAL_DEBUG_WIFI_MANAGER_
            Serial.println("->Connect best WiFi FAIL");
        #endif
    }
}
...
{
    #ifdef _SERIAL_DEBUG_WIFI_MANAGER_
        Serial.print("#WiFi remove current WiFi info: "); 
        Serial.println(WiFi.SSID());
    #endif 
