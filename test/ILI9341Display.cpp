...
void drawSdJpeg(const char *filename, int xpos, int ypos)
{
    ...
    if (!jpegFile)
    {
        #ifdef _SERIAL_DEBUG_ILI9341_DISPLAY_
            Serial.print("[E]File \"");
...
void ILI9341Display::progressScreen(bool _sd, bool _opcn3, bool _sht85, bool _als_toxic_no2_o3, bool _als_toxic_co_so2, bool _ds3231)
{
#ifdef _SERIAL_DEBUG_ILI9341_DISPLAY_
    ...
    {
        tft.setTextColor(TFT_WHITE);
        tft.println("Alphasense NO2 & O3 is connected");
    }
    else
    {
        tft.setTextColor(TFT_RED);
        tft.println("Alphasense NO2 & O3 is disconnected");
    }
    if (_als_toxic_co_so2)
    {
        tft.setTextColor(TFT_WHITE);
        tft.println("Alphasense CO & SO2 is connected");
    }
    else
    {
        tft.setTextColor(TFT_RED);
        tft.println("Alphasense CO & SO2 is disconnected");
    }
    if (_sht85)
    {
        tft.setTextColor(TFT_WHITE);
    ...
    {
        tft.setTextColor(TFT_WHITE);
        tft.println("OPCN3 is connected");
    }
    else
    {
        tft.setTextColor(TFT_RED);
        tft.println("OPCN3 is disconnected");
    }
    if (_sd)
    {
        tft.setTextColor(TFT_WHITE);
...
void ILI9341Display::updateData(DataCore _data_core, char *_mac)
{
#ifdef _SERIAL_DEBUG_ILI9341_DISPLAY_
    ...
    tft.printf("%.1f", _data_core.temperature);
    ...
    tft.printf("%.1f", _data_core.humidity);
    tft.setCursor(195, 206, 2);
    tft.print("%");

    //Pressure
    tft.fillRect(244, 200, 50, 20, TFT_WHITE);
    tft.setCursor(244, 200, 4);
    tft.printf("%.1f", _data_core.pressure / 101325.0);
    ...
    //NO2 O3 CO SO2
    tft.fillRect(190, 100, 110, 90, TFT_WHITE);
    tft.setCursor(165, 100, 2);
    tft.printf("NO2: %.3f-%.3f", _data_core.NO2_WmV, _data_core.NO2_AmV);
    tft.setCursor(165, 125, 2);
    tft.printf("O3 : %.3f-%.3f", _data_core.O3_WmV, _data_core.O3_AmV);
    tft.setCursor(165, 150, 2);
    tft.printf("CO : %.3f-%.3f", _data_core.CO_WmV, _data_core.CO_AmV);
    tft.setCursor(165, 175, 2);
    tft.printf("SO2: %.3f-%.3f", _data_core.SO2_WmV, _data_core.SO2_AmV);
