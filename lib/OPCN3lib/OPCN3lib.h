...
private:
    ...
    ResponseType sendCommand(byte inputByte, byte commandByte, int outputSize)
    {
        ...
        byte valid_check = 0xFF; //this is for if all data byte received equals to 0xFF
        beginTransfer();
        initial[0] = opcnSPI.transfer(inputByte);
        delay(10);
        initial[1] = opcnSPI.transfer(inputByte);
        for (int i = 0; i < outputSize; i++)
        {
            delayMicroseconds(10);
            dataIn[i] = opcnSPI.transfer(commandByte);
        }
        endTransfer();
        ...
        for(uint8_t i = 0; i< outputSize; i++){
            valid_check = valid_check & dataIn[i];
        }
        if(valid_check == 0xFF){
            response.valid = 0;
        }
